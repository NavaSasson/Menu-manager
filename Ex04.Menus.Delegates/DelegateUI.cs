...
namespace Ex04.Menus.Delegates
{
    internal class DelegateUI
    {
        internal int GetValidItemChoice(MenuItem i_MenuItem, List<MenuItem> i_MenuItemsList)
        {
            ...
            bool isInOptions = isChoiceInMenuOptions(i_MenuItem, userChoiceInteger, i_MenuItemsList);
            if (!isInOptions)
            {
                Console.WriteLine("Invalid choice. Please try again.");
        ...
        {
            string menuTitle = i_MenuItem == null ? "**Delegates Main Menu**" : $"**{i_MenuItem.Title}**";
